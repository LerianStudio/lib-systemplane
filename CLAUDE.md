...
- **Single-tenant** (default). The constructor receives a non-nil `*sql.DB` / `*mongo.Client`. Reads serve from the engine's cache; a write upserts through the store and publishes into that cache before returning, so a caller reads its own write. The backend's changefeed (LISTEN/NOTIFY on Postgres, change stream on MongoDB) drives invalidation, and every (re)connect is answered with a full reconcile of the scope, so a value written while the feed was down converges without a second write. Subscribers registered before `Start` receive exactly one delivery per registered key during `Start` — the registered default at revision 0 for a key with no row.
