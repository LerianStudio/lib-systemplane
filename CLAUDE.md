...
- **Multi-tenant with a tenant manager** (`WithPostgresTenantManager(mgr)` / `WithMongoTenantManager(mgr)`; each implies `WithMultiTenantEnabled()`, and the other backend's constructor refuses it with `ErrTenantManagerBackendMismatch`). The middleware stays: writes and uncached reads use the database it resolved, so `mgr` must be the Manager it registers under the `WithModule` name. `Start` activates nothing. A tenant's first read (tenant from `tmcore.GetTenantIDContext`) activates that tenant's scope in the background; once it is up its reads serve from the engine's cache, fed by the tenant's own changefeed (one LISTEN connection per active tenant per replica on Postgres). `OnChange` covers every tenant with `Change.Tenant` set, and every activation announces every registered key. `HandleTenantLifecycle` (the `tmevent.EventHandler` signature, chained after the dispatcher's `HandleEvent`) unblocks on `tenant.activated`, drops and blocks the scope on `tenant.suspended` / `tenant.deleted` (reads stay per request), rebuilds an active scope on `tenant.credentials.rotated`, and returns only `ErrClosed` and `ErrValidation`. A tenant feed that would share a database (Postgres) or a database and collection (MongoDB) with another tenant's live feed is refused at activation: a WARN, reads stay per request, no root sentinel. Consumer-facing semantics: [MIGRATION-v4.md § notifications](MIGRATION-v4.md#notifications).
