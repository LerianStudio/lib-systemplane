...
| 3 | Nothing in the library masks, redacts or withholds a value: no key policy, admin serves values in clear, the catalog drops `redaction` (Epic 3.1, removal, product decision 2026-09-24; FC-13 field-level redaction cancelled). Epic 3.3 (group validator sees the `Set` context) rides in this phase unchanged | 3.1, 3.3 | Detailed |
...
## Phase 3: Remove redaction

**Cancelled: field-level redaction (FC-13).** Cancelled by Fred's product decision on 2026-09-24; FC-13 is retired in `index.md`. Branch `feat/v4-groups-field-redaction` (Epic 3.1 of the cancelled phase, five commits, never merged) is discarded and none of it is carried forward.

The decision, verbatim: "devemos retirar 100% o tema de segredos mascarados. nao faz sentido a systemplane mascarar nada. pode retirar e limpar tudo." lib-systemplane holds runtime-mutable knobs, and secrets never live in it (the scope rule in `CLAUDE.md` and README § Scope). Nothing in the library therefore masks, redacts or withholds a value on behalf of a "secret" key: no key policy, no masked admin response, no withheld log line, error or panic report. Epic 3.1 below deletes the whole theme. Epic 3.2 was the admin half of FC-13 and is retired with it; Epic 3.3 is unrelated to redaction and stays as written.

**What stays, and is not this theme.** Two things spell "redact" in this repo and survive the epic, because neither is about a key's value:

- **The generic log-hygiene rule.** The library never adds a stored value to a log line of its own. A validation refusal logs the validator's error (`log.Err(err)`) and never the value; decode failures log the decoder's error the same way. It predates redaction and every task below keeps it.
- **lib-observability's field-name redaction.** lib-observability's logger replaces the value of a field whose NAME is on its sensitive-field list (`key` is on it) with `[REDACTED]`. That is why every line names a key in field `keyname`, and it is what `internal/testsupport/logguard` and the `TestNoLoggedFieldNameIsRedacted` / `*_SurviveRedaction` tests guard. It stays, and so does the import `github.com/LerianStudio/lib-observability/v4/redaction` those tests use. lib-observability's production mode, which omits a recovered panic value from the report (`TestCoordinatorApplierPanicIsRedactedInProductionMode`), is the same library's behaviour and stays too.

**Line references** are to `develop` at `07ba7fb` (after PR #96). PR #97 edits `CLAUDE.md`, `api_constructors.go` and `api_client_test.go`; re-read those at the rebase and move the references, never the scope.

### Epic 3.1: Delete redaction from the library
...
**Admin HTTP contract (breaking change of the v4 admin surface; v4 is pre-release).** After 3.1.1, `GET :prefix/:namespace/:key`, `GET :prefix/:namespace` and the catalog detail return the stored value and the registered default as they are, never `constants.ObfuscatedValue`. After 3.1.4, every catalog key summary and detail (`CatalogKeySummary` and the `CatalogKeyDetail` that embeds it) loses its `redaction` JSON field. `CatalogVersion` stays `"systemplane.catalog.v1"`: no reader of the field is known, and the removal ships inside the v4 major. MIGRATION-v4.md names both changes (Task 3.1.5).

**Epic done when:**
- `git grep -n -E 'RedactPolicy|RedactNone|RedactMask|RedactFull|WithRedaction|KeyRedaction|ApplyRedaction|AnyRedacted|WithheldPanic|ErrorDetail|withheldValueErr|\.Redacted\b|registered redacted|value withheld|cause withheld|ObfuscatedValue' -- ':!docs/plans' ':!CHANGELOG*'` returns nothing.
- `git grep -il redact -- ':!docs/plans' ':!CHANGELOG*'` lists exactly the lib-observability field-name guard and nothing else: `internal/testsupport/logguard/logguard.go`, `internal/testsupport/logguard/logguard_test.go`, `internal/testsupport/logguard/testdata/fields/source.go`, `internal/client/logging_test.go`, `internal/engine/ingest.go` (the `logWarn` doc comment on field `keyname`), `internal/engine/logging_test.go`, `internal/group/status_test.go`, `internal/mongodb/mongodb.go` (the `logFieldKeyName` comment), `internal/mongodb/mongodb_obs_test.go`, `internal/postgres/postgres_obs_test.go`; and every remaining hit in those files refers to lib-observability's field-name redaction or its production mode.
- `make ci` is green on `refactor/v4-drop-redaction`.
...
**Implementation vision:** Delete, in `admin/admin.go`: the two `policy := client.KeyRedaction(...)` / `redacted := systemplane.ApplyRedaction(...)` pairs (`:295-296`, `:433-434`), rendering `entry.Value` / `e.Value` directly into `Value`; the two catalog-detail lines `:331-332`, leaving `detail.DefaultValue` as `CatalogKey` returned it; the function `catalogRedactionPolicy` (`:409-418`). After this task admin calls none of `KeyRedaction`, `ApplyRedaction`, `RedactPolicy`, `RedactNone`, `RedactMask`, `RedactFull`. Tests in `admin/admin_test.go`: delete `TestAdmin_CatalogRedactsDefaultValue` (`:744-788`), `TestAdmin_CatalogDoesNotRedactExamples` (`:789-823`), `TestAdmin_GetOneRedactsValueWithProvenance` (`:995-1038`) and `TestAdmin_ListRedactsValues` (`:1139-1195`). Provenance stays pinned by `TestAdmin_GetOneCarriesRevisionAndProvenance` (`:899`) and `TestAdmin_ListCarriesRevisionAndProvenancePerEntry` (`:1040`), and raw catalog examples by `TestAdmin_CatalogListAndDetail` (`:493`). Drop any import that becomes unused (`obsconstants`).

**Files:**
- Modify: `admin/admin.go`
- Test: `admin/admin_test.go`

**Verification:** `go build ./... && go vet -tags=unit ./admin/... && go test -tags=unit -race ./admin/... && git grep -n -i redact -- admin/` (empty).

**Done when:** `git grep -n -i redact -- admin/` returns nothing; the admin suite and `admin/release_policy_test.go` are green; a key's stored value and registered default reach the GET, list and catalog-detail bodies unchanged.

**Estimated size:** ~12 turns.
...
**Implementation vision:** Delete, in `api_group.go`: the comment block and the `redacted :=` line (`:174-182`), the `redacted` argument at `:189`, and the two FC-13 sentences of `seedCurrentEntry`'s doc (`:262-265`, "FC-13's Phase 3 reworks the catalog, and the groups-redaction lane replaces this with a direct probe of the Client's mode then."). The `CatalogKey` probe itself stays: it answers the mode question correctly, and no lane is left that wants it replaced. In `internal/group/coordinator.go`: delete the `redacted` field and its doc, the `redacted` parameter of `NewCoordinator` (new signature `NewCoordinator[T](logger, namespace, key string, multiTenant bool, decode, seed)`) and its assignment; replace the three `safelog.ErrorDetail(c.redacted, ...)` fields with `log.Err(err)` (`seeded.decodeErr` at `:433`); in `reportPanic` delete `reported` and the `if c.redacted` branch and pass `recovered` straight to `runtime.HandlePanicValue`. Rewrite the doc comments at `:176-182`, `:213-215`, `:794`, `:814-818`, `:831-833` and `:864-874` without the redaction clauses: namespace and key are carried because a report must name its group; `ErrApplyPanicked` carries no part of the panic value because `LastErr` is a field operators read and the value belongs to the panic handler; a returned apply error is logged as the error, never with the document added. Tests: delete `api_group_redaction_test.go` and `internal/group/redaction_test.go`; in `internal/group/status_test.go` delete `decodeRedactionCases` (`:167-176`), drop the `redacted` parameter of `assertDecodeFailureRendering` (`:178-220`) and of `assertRejectionRendering` (`:476`) keeping only the verbatim-error branch, and flatten their loops at `:441` and `:585`; in `internal/group/seed_test.go` flatten the loop at `:220` and the call at `:253`; drop the `redacted` argument from every `NewCoordinator` call (8 in `coordinator_test.go`, 3 in `seed_test.go`, 12 in `status_test.go`); in `api_group_test.go` `TestGroupBindForwardsKeyOptions` (`:1180-1206`) keeps the `WithDescription` assertion and loses `WithRedaction`, the `KeyRedaction` assertion and the comment sentence about a credential group. `TestNoLoggedFieldNameIsRedacted` (`:230`) and `TestCoordinatorApplierPanicIsRedactedInProductionMode` (`:352`) stay: they are lib-observability's field-name and production-mode behaviour.

**Files:**
- Modify: `api_group.go`, `internal/group/coordinator.go`
- Delete: `api_group_redaction_test.go`, `internal/group/redaction_test.go`
- Test: `api_group_test.go`, `internal/group/coordinator_test.go`, `internal/group/seed_test.go`, `internal/group/status_test.go`

**Verification:** `go build ./... && go test -tags=unit -race ./internal/group/... . -run 'Group|Bind|Coordinator' && git grep -n -E 'redacted|KeyRedaction|WithheldPanic|ErrorDetail|FC-13' -- api_group.go internal/group/` (empty).

**Done when:** the group coordinator has no redaction input; a group's decode failure, apply rejection and apply panic are reported with the error or panic value as produced; the grep above is empty; `make test-unit` green.

**Estimated size:** ~22 turns.
...
**Implementation vision:** Delete `Registry.AnyRedacted`, `KeyDef.Redacted` and `anyRedacted`. Drop the `redacted` parameter from `RunValidator`, `runValidator`, `reportConsumerPanic` and `logValidatorRejection`; `reportConsumerPanic` hands `recovered` to `reportRecovered` unchanged. In `dispatch.go` and `feed.go` delete the `def, _ := e.lookup(...)` lines that existed only to read the bit. In `reconcile.go` delete the gate comment and the inner `defer safelog.Swallow()` (the engine logger is guarded by `log.Guard` at `engine.go:131` and `reportRecovered` swallows its own panics). Replace `safelog.ErrorDetail(..., err)` with `log.Err(err)` at `ingest.go:187`, `:233`, `internal/client/get.go:125` and `:386`. Delete `WithheldPanic` and `ErrorDetail` from `internal/safelog/safelog.go` and rewrite the package doc: safelog holds `UnresolvedTenant` and `Swallow`, shared by `internal/engine` and `internal/group`. Rewrite, without redaction: `logValidatorRejection`'s doc (`ingest.go:215-227`) as the log-hygiene rule — the line carries the validator's error and never the value, and the error returned to the caller is unchanged; `RunValidator`'s (`:306-309`), `runValidator`'s (`:357-363`, `:377-379`) and `reportConsumerPanic`'s (`:395-424`) docs; `dispatch.go:365-375`; `feed.go:421-425`; `engine/doc.go:19` ("the key registry, options and catalog stay there"). `internal/client`: delete `Lookup`'s `Redacted:` line and its doc sentence, and `AnyRedacted` with its doc; drop the last argument at `register.go:111` and `set.go:86` (the `redacted` local at `get.go:102` stays until Task 3.1.4, which deletes `decodeErr`'s parameter). Tests: `internal/engine/ingest_test.go` delete `fakeRegistry.AnyRedacted` (`:30-38`), `TestValidatorPanicOnARedactedKeyWithholdsTheValue` (`:777-`, including the `safelog.ErrorDetail` comment at `:782`) and the `false` argument at `:758`; `internal/engine/dispatch_test.go` delete `deadRegistry.AnyRedacted` (`:147`) and `TestOnChangePanicOnARedactedKeyWithholdsTheValue` (`:1116-`, fixture `Redacted: true` at `:1133`); `internal/engine/logging_test.go` delete `TestUndecodableValueIsRedactedByKeyPolicy` (`:628-693`), `TestReReadPanicOnARedactedKeyWithholdsTheValue` (`:1522-1581`) and `TestReconcilePanicOnARedactedRegistryWithholdsTheValue` (`:1582-`), and reduce `TestValidatorErrorIsRedactedByKeyPolicy` (`:576-627`) to its "keeps the validator's own message" case, renamed `TestValidatorRejectionLogsTheErrorNotTheValue`: the line carries the validator's error, and a value the validator did not name appears nowhere in it; `internal/safelog/safelog_test.go` is deleted (its two helper tests go with the helpers, and a field-name guard over a package that writes no field guards nothing); `internal/client/registry_test.go` drop the `redacted` column and assertion (`:16-49`) and delete `TestRegistryAnyRedacted` (`:124-160`); `internal/client/onchange_engine_test.go` delete `panicSentinel` (`:1106-1108`) and `TestAPanickingValidatorOnARedactedKeyWithholdsTheValue` (`:1110-`); `internal/client/logging_test.go` flatten `TestMultiTenantDecodeFailureWithNoTenantIDSaysSo`'s policy loop (`:254-`) to one case that registers without `WithRedaction`, and drop "and a redacted one alike" from its doc (`:252`); `internal/client/redaction_log_test.go` delete `TestMultiTenantDecodeFailureHonorsTheKeyRedaction` (`:61-178`) and `TestSingleTenantDecodeFailureWithholdsTheRedactedRow` (`:179-252`) (the rest of the file goes in Task 3.1.4); `internal/client/readback_validator_test.go:631` comment says "leaves the value to lib-observability's recovery pipeline". `TestRunValidator`, the `keyname` guards (`requireNotRedacted`, `TestNoLoggedFieldNameIsRedacted`) and every other test stay.

**Files:**
- Modify: `internal/engine/registry.go`, `internal/engine/ingest.go`, `internal/engine/dispatch.go`, `internal/engine/feed.go`, `internal/engine/reconcile.go`, `internal/engine/doc.go`, `internal/safelog/safelog.go`, `internal/client/registry.go`, `internal/client/register.go`, `internal/client/set.go`, `internal/client/get.go`
- Delete: `internal/safelog/safelog_test.go`
- Test: `internal/engine/ingest_test.go`, `internal/engine/dispatch_test.go`, `internal/engine/logging_test.go`, `internal/client/registry_test.go`, `internal/client/onchange_engine_test.go`, `internal/client/logging_test.go`, `internal/client/redaction_log_test.go`, `internal/client/readback_validator_test.go`

**Verification:** `go build ./... && go vet -tags=unit ./... && go test -tags=unit -race ./internal/engine/... ./internal/safelog/... ./internal/client/... && git grep -n -E 'AnyRedacted|anyRedacted|\.Redacted\b|Redacted:|WithheldPanic|ErrorDetail|value withheld' -- internal/` (empty), then `go test -tags=unit -run=^TestPerf_ ./...`.

**Done when:** the engine port and `internal/safelog` carry no redaction symbol; every recovered consumer panic reaches lib-observability's handler with the value it was raised with; every decode and validator line carries `log.Err(err)`; the grep above is empty; `make test-unit` and the perf gate green.
...
**Implementation vision:** Delete `internal/client/redact.go`; the `redaction` field of `keyDef` and its default; `WithRedaction` in `internal/client/options.go`; `KeyRedaction` in `get.go`; `withheldValueErr`; the `RedactPolicy` return of `getEntry` (new signature `getEntry(ctx, namespace, key string) (Entry, bool, error)`, doc cut to the read path) and its three callers' second blank; the `policy != RedactNone` branches in `GetInt` and `GetDuration`, which keep their existing value-naming errors; the `redacted` parameter of `decodeErr`, which always wraps the cause with `%w`, and the doc sentences about it (`client_telemetry.go:77-84`); the `Redaction` field of `CatalogKeySummary` and its assignment. Rewrite `catalog.go:57` and `api_types.go:38` to "Examples are emitted as provided; never include secrets or credentials." and `internal/client/doc.go:4` without "redaction". Root: delete `RedactPolicy`, `RedactNone`, `RedactMask`, `RedactFull`, `(RedactPolicy).String`, `WithRedaction`, `ApplyRedaction`, `(*Client).KeyRedaction`; `api_constants.go` keeps only `CatalogVersion`. `boundary_test.go` needs no change. Tests: delete `internal/client/redaction_log_test.go`; `internal/client/testing_facade_test.go` delete `TestRedactionHelpers` (`:191-`); `internal/client/client_test.go` `TestKeyDescriptionAndRedaction` (`:731-`) becomes `TestKeyDescription` without `WithRedaction` or the `KeyRedaction` assertion; `internal/client/catalog_test.go` drop `WithRedaction(RedactMask)` (`:101`) and the `summary.Redaction` check (`:140-141`); `internal/client/registry_test.go` drop the remaining `policy` column and `WithRedaction` (`:16-33`); `api_client_test.go` delete `TestPublicKeyRedactionFailsClosed` (`:134-163`), the `KeyRedaction` assertion (`:200-202`) and the `ApplyRedaction` / `RedactPolicy` block of `TestPublicConstructorsAndOptions` (`:289-301`); `api_catalog_test.go` drop `WithRedaction(RedactFull)` (`:41`) and the `Redaction` check (`:63`). The catalog JSON loses `redaction` here; `CatalogVersion` is unchanged (see the epic's contract note).

**Files:**
- Modify: `internal/client/options.go`, `internal/client/register.go`, `internal/client/get.go`, `internal/client/client_telemetry.go`, `internal/client/catalog.go`, `internal/client/doc.go`, `api_types.go`, `api_constants.go`, `api_constructors.go`, `api_client.go`
- Delete: `internal/client/redact.go`, `internal/client/redaction_log_test.go`
- Test: `internal/client/testing_facade_test.go`, `internal/client/client_test.go`, `internal/client/catalog_test.go`, `internal/client/registry_test.go`, `api_client_test.go`, `api_catalog_test.go`

**Verification:** `go build ./... && go vet ./... && go vet -tags=unit ./... && make test-unit && go test -tags=unit -run=^TestPerf_ ./...`, then the epic's two greps restricted to `-- '*.go'`: the symbol grep empty, the broad grep listing only the lib-observability field-name files.

**Done when:** the root package exports no redaction symbol (`go doc -all . | grep -i redact` is empty); a typed getter's error and a multi-tenant decode error name the value or wrap the cause for every key; the catalog key summary has no `redaction` field; `make ci` green.
...
**Implementation vision:** Rebase onto `develop` after the docs Phase 1 PR merges and re-read every file above there. `CLAUDE.md`: delete `KeyRedaction` from the listing bullet, "redaction policy (`RedactNone | RedactMask | RedactFull`)" and `WithRedaction` from the registered-keys bullet (with the fail-closed sentence), "redaction" from the lib-observability and `internal/client` descriptions, and rewrite the `internal/safelog/` bullet as "the tenant stamp for an unresolved tenant and the swallow for a panicking consumer logger, shared by `internal/engine` and `internal/group`"; restate in the registered-keys bullet that a refusal logs the error and never the value. `README.md`: drop "redaction" from the two capability lists and the catalog sentence; delete the admin-obfuscation sentence at `:369` and keep "do not put secrets … in registered defaults, persisted values, schemas, rules, or examples. Systemplane is not a secret store."; reword `:385` to "omits the recovered value from the log line"; replace `:391`'s last sentence with "Nothing in systemplane masks a value: a value stored here is readable by every caller the admin authorizer allows, and appears in logs and errors." `docs/PROJECT_RULES.md`: drop "redaction" from `:326`; replace `:357-370` with the log-hygiene rule (the library never adds a value to its own log line; a refusal logs the error) and lib-observability's field-name rule (name a key `keyname`, never `key`). `.env.reference`: delete the `WithRedaction (RedactPolicy constant)` item. `MIGRATION-v4.md`: delete § Redaction fails closed; delete the redacted-key clauses at lines 180 and 444; remove `KeyRedaction` and `WithRedaction` from the "unchanged" paragraph; add to the "Removed" table the rows `RedactPolicy`, `RedactNone`, `RedactMask`, `RedactFull` | "No replacement. systemplane holds runtime knobs, never secrets; move a secret to environment variables or a secret manager." ; `WithRedaction`, `ApplyRedaction`, `(*Client).KeyRedaction` | "No replacement; drop the option from every `Register` / `Bind` call, including `WithRedaction(RedactNone)`."; and a § Behaviour changes entry "Nothing is masked any more": admin GET, list and catalog detail return values and defaults in clear, the catalog loses its `redaction` field, and typed-getter errors, decode errors, validator lines and panic reports carry the value or error as produced. Every § Per consumer entry whose consumer registers with `WithRedaction` gains the drop step (go-boilerplate-ddd and the services generated from it register every key with `WithRedaction(RedactNone)`; matcher registers four `RedactFull` keys). No other section changes.

**Files:**
- Modify: `README.md`, `CLAUDE.md`, `MIGRATION-v4.md`, `docs/PROJECT_RULES.md`, `.env.reference`

**Verification:** `git grep -n -i redact -- README.md CLAUDE.md MIGRATION-v3.md MIGRATION-v4.md doc.go docs/PROJECT_RULES.md .env.reference` returns only the MIGRATION-v4 `keyname` section's `key=[REDACTED]`; the epic's two greps; `go build ./... && go vet ./...` (doc.go unchanged but compiled).

**Done when:** no doc describes a redaction policy, a masked value or a withheld report; MIGRATION-v4.md's Removed table carries the redaction rows and its Behaviour changes names the unmasked admin surface; the epic's Done-when greps pass.

**Estimated size:** ~15 turns.

### Epic 3.2: Render per field on the admin surface

**Status:** Retired with FC-13 (Fred, 2026-09-24).
...
Five assertions this lane cannot make green on its own base, each because it depends on engine behavior that has not landed:
...
5. Multi-tenant per-tenant delivery through a bound Manager, end to end — a publication of tenant A and one of tenant B reaching the same group's applier as two scopes with their own `Applied.Tenant`, `Previous` and `Status` row. The Manager's callbacks come off per-tenant LISTEN goroutines, so end to end needs a real Postgres and two tenant databases. This lane pins the hop it owns in-package instead (`(*Group).publish`, `api_group_publish_test.go`): `Change.Tenant` -> `Publication.Tenant` -> `Decoded.Tenant` -> `Applied.Tenant`, two tenants, two Status rows. Everything upstream of that hop — that the feed names the right tenant on the right goroutine — belongs to the integration lane's scenario list.
