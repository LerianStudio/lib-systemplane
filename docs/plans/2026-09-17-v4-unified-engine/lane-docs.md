...
**Progress 2026-09-25 (branch `docs/v4-examples`).** The `examples` job landed, mirroring `perf-gate`. The godoc sweep waits until `engine-tenants` merges: it reads `api_*.go` and `internal/client/options.go`, which that lane is still writing, so a sweep now reports a surface that is about to change.
