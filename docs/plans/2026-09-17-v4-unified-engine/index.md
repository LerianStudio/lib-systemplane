...
- A Postgres tenant connector whose DSN pins a `search_path` (one schema per tenant inside a shared database) is refused at `Subscribe` with `ErrSchemaIsolationUnsupported` (storage fix passes 2 and 3, on the tenant DSN and on `ListenDSN` alike): NOTIFY is database-wide, so v4 requires one database per tenant.
