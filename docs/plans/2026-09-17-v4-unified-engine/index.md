...
- `(*Client).KeyRedaction` answers `RedactFull` for a closed or nil Client, where v3 answered `RedactNone`: a consumer that renders values under this policy after `Close` now masks instead of disclosing. An unregistered key on an open Client still reports `RedactNone` (`BREAKING CHANGE` footer of `fix(client)!: fail closed when a closed Client reports a key's redaction`).
