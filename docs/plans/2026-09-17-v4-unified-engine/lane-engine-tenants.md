...
- D-P3-6: a stopped MongoDB feed leaves its server-side change-stream cursor open until the server cursor timeout (10 min by default): `internal/mongodb` cancels the ctx of the blocked `Next`, and mongo-driver v2.9 then closes the cursor with that cancelled ctx, so `killCursors` never goes out. Resolved by `fix(mongodb): kill the change-stream cursor when a feed stops`: the reader loops on `TryNext`, checks the stop between reads and closes the stream through the driver, so the Mongo suspension census now counts idle cursors too. Known limit, no code: under the documented wiring the dispatcher's `HandleEvent` runs first and disconnects a suspended, deleted or rotated tenant's client before the engine stops the feed, so each such event leaves one cursor until the server timeout reaps it.
