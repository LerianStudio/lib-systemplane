...
func TestDeliveryNeverReadsTheRegistry(t *testing.T) {
	...
	var panicked recorder

	unsubPanicking := e.OnChange(nk, func(ctx context.Context, ch Change) {
		panicked.record(ctx, ch)
	...
	waitFor(t, hangGuard, "the panicking delivery", func() bool { return panicked.len() == 1 })

	if got := reg.lookups.Load(); got != 0 {
		t.Errorf("delivery took %d registry lookup(s), want 0", got)
	}
}
